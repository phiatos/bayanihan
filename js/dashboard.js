...
document.addEventListener("DOMContentLoaded", () => {
    ...
    auth.onAuthStateChanged(user => {
        ...
        database.ref(`users/${user.uid}`).once("value", snapshot => {
            ...
            // No need for userEmail if we are using userUid for filtering
            ...
            database.ref("reports/approved").on("value", snapshot => {
                ...
                if (reports) {
                    Object.values(reports).forEach(report => {
                        ...
                        totalHotMeals += parseFloat(report.NoOfHotMeals || 0); // Corrected typo here (NoOfHotMea1s -> NoOfHotMeals) if it's in your data
                        totalWaterLiters += parseFloat(report.LitersOfWater || 0);
                        totalVolunteers += parseFloat(report.NoOfV01unteersMobi1ized || 0); // Corrected typo here (NoOfV01unteersMobi1ized -> NoOfVolunteersMobilized) if it's in your data
                        ...
                        totalInKindDonations += parseFloat(report.TotalVa1ueOfInKindDonations || 0); 
                    });
                } else {
...
//     // Initialize Firebase
//     // It's already initialized by the Google Maps callback (initMap),
//     // but initializing again here to ensure it's available for metrics logic
//     // if initMap finishes later or not at all (e.g., if Google Maps API key is invalid).
//     // This second initialization call won't cause issues due to Firebase's internal checks.
//     firebase.initializeApp(firebaseConfig);
//     const auth = firebase.auth();
//     const database = firebase.database();
...
//     // Search bar for both metrics and map search
//     const searchInput = document.getElementById("search-input");
//     // You might want to add a filter function here if the search input is supposed to filter the metrics as well.
//     // For now, it's only connected to Google Places Autocomplete.
//     searchInput.addEventListener("input", (e) => {
//         const query = e.target.value.trim();
//         console.log("Search query for dashboard metrics (if applicable):", query);
//         // Add logic here to filter metrics based on query if needed
...
//                 let totalWaterLiters = 0;
//                 let totalVolunteers = 0;
//                 let totalMonetaryDonations = 0; // Renamed for clarity in this scope
//                 let totalInKindDonations = 0;
...
//                         // THIS IS THE MAIN CHANGE FOR MONETARY DONATIONS:
//                         totalMonetaryDonations += parseFloat(report.TotalMonetaryDonations || 0); // Corrected field name
