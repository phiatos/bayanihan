...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // This is where we integrate the auth.onAuthStateChanged listener
    auth.onAuthStateChanged(user => {
        ...
        // Now that the user is authenticated, we can safely initialize page functions
        initializePageFunctions(user.uid);
        resetInactivityTimer(); // Start the inactivity timer only when authenticated
    });
});

// Wrap the rest of your DOMContentLoaded logic into a new function
// This function will only be called IF the user is authenticated
function initializePageFunctions(userId) { // userId is passed if needed for functions
    const volunteerOrgsContainer = document.getElementById('volunteerOrgsContainer');
    const searchInput = document.getElementById('searchInput');
    const sortSelect = document.getElementById('sortSelect');
    const entriesInfo = document.getElementById('entriesInfo');
    ...
    // --- Initial Load ---
    fetchPendingApplications(); // This will now only be called if authenticated
